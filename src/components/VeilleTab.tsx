...
export const VeilleTab = () => {
  ...
  const veilleArticles = sources.filter(s => s.category === 'VEILLE' || s.category === 'RECHERCHE');

  const handleTranslate = async (id: string, title: string, abstract: string) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between pt-6 border-t border-[#F5F4F0] mt-auto">
                  <button 
                    onClick={() => handleTranslate(article.id, article.title, article.abstract)}
